...
var NON_INTERACTIVE_CLIENT_ID = 'Gr3fjGTz1UpZEQLG2ZheNQvCX3vQ7PyW';
var NON_INTERACTIVE_CLIENT_SECRET = 'ikF_sG_FX1G1r5f_7ifhHzPXWt0vU8IqqlS6N4qo7EbmALK2HbLzn5F6djBbAQKG';

var authData = {
  client_id: NON_INTERACTIVE_CLIENT_ID,
  client_secret: NON_INTERACTIVE_CLIENT_SECRET,
  grant_type: 'client_credentials',
  audience: 'https://movieanalyst.herokuapp.com/'
}
...
app.get('/authors', getAccessToken, function(req, res){
  request
    .get('http://localhost:9000/reviewers')
    .set('Authorization', 'Bearer ' + req.access_token)
    .end(function(err, data) {
      if(data.status === 403){
        res.send(403, '403 Forbidden');
...
app.get('/publications', getAccessToken, function(req, res){
  request
    .get('http://localhost:9000/publications')
    .set('Authorization', 'Bearer ' + req.access_token)
    .end(function(err, data){
      if(data.status === 403){
        res.send(403, '403 Forbidden');
...
app.get('/pending', getAccessToken, function(req, res){
  request
    .get('http://localhost:9000/pending')
    .set('Authorization', 'Bearer ' + req.access_token)
    .end(function(err, data){
      if(data.status === 403){
        res.send(403, '403 Forbidden');
